...
// FIX 1: localStorage helper for persistence
function ls(key, def) {
...
function calcCostos(salario, arlRate, subsidio, categoria = "empleado") {
  const s = +salario || 0;
  // Prestación de servicios: la empresa solo paga el honorario pactado, sin carga social
...
// FIX 3: Commission calculation — two modes
// "por_tramo": el % se aplica al TOTAL de ventas según el tramo en que cae (ej: 200M * 1% = 2M)
// "marginal":  el % se aplica solo al tramo incremental (ej: solo los 190M por encima de 10M)
...
// FIX 2: AddForm handles both Add and Edit
...
function PresupuestoTab({ employees, beParams }) {
  ...
  const mc=pv-cv, mcPct=pv>0?mc/pv:0;
  const totalNomina=employees.reduce((s,e)=>s+calcCostos(e.salario,e.arlRate,e.subsidio,e.categoria).costoTotal,0);
  ...
  return (
    <div>
      ...
      {(!pv||!cv)&&<div className="hbox mb14">⚠️ Configura <strong>Precio de Venta</strong> y <strong>Costo Variable</strong> en la pestaña "Punto de Equilibrio" para ver los presupuestos exactos.</div>}
      <div className="card mb14">
        <div className="g3">
...
function BreakevenTab({ beParams, setBeParams, totalCostosFijos }) {
  ...
  const pv=+beParams.precioVenta||0,cv=+beParams.costoVariable||0,otros=+beParams.otrosCostos||0;
  const cf=totalCostosFijos+otros,mc=pv-cv,mcPct=pv>0?mc/pv:0;
  const peU=mc>0?cf/mc:null,peI=mcPct>0?cf/mcPct:null;
  const mgRent=+beParams.margenRentabilidad||0;
  const peIConRent=mcPct>0&&(mcPct-mgRent/100)>0?cf/(mcPct-mgRent/100):null;
  return (
    <div>
        ...
        <div>
          <div className="card">
            ...
            <div className="hbox" style={{marginBottom:12}}>
              <div className="sm muted mb8">Costos de nómina (auto, suma de empleados)</div>
              <div className="mono acc" style={{fontSize:22,fontWeight:700}}>{fmtCOP(totalCostosFijos)}</div>
            </div>
            <div className="ig"><label className="il">Otros Costos Fijos (arrendamiento, servicios...) COP</label><input className="inp" type="number" value={beParams.otrosCostos} onChange={set("otrosCostos")} placeholder="0"/></div>
          ...
          <div className="card">
            ...
            {pv>0&&cv>0&&<div className="g2"><div className="stat"><div className="stat-l">Margen contrib. unitario</div><div className="stat-v g">{fmtCOP(mc)}</div></div><div className="stat"><div className="stat-l">Margen contrib. %</div><div className="stat-v g">{fmtPct(mcPct*100)}</div></div></div>}
          </div>
        </div>
        <div>
          ...
          {mgRent>0&&<div className="pe-box" style={{background:"linear-gradient(135deg,rgba(96,165,250,.1),rgba(96,165,250,.03))",border:"1px solid rgba(96,165,250,.3)"}}>
            <div className="sm muted" style={{textTransform:"uppercase",letterSpacing:.5,marginBottom:6}}>🎯 Meta con {fmtPct(mgRent)} de Rentabilidad</div>
            <div className="big-num blu">{peIConRent?fmtCOP(peIConRent):"—"}</div>
            <div className="sm muted mt8">ventas necesarias para lograr el margen deseado</div>
          </div>}
          {peU&&<div className="card">
            <div className="card-title">Verificación en el PE</div>
            {[["Ingresos totales",fmtCOP(Math.ceil(peU)*pv),"var(--accent)"],["Costos variables",fmtCOP(Math.ceil(peU)*cv),"var(--red)"],["Costos fijos",fmtCOP(cf),"var(--red)"]].map(([l,v,c])=>(
...
function ComisionesTab({ globalSchema, setGlobalSchema, schemas, setSchemas, employees, tipoEsquema, setTipoEsquema }) {
  ...
  return (
    <div>
      ...
      {/* FIX 3: Toggle tipo de comisión */}
      <div className="card mb14">
        <div className="card-title">Modelo de Cálculo de Comisión</div>
        <div className="toggle-row">
          <button className={`toggle-btn${tipoEsquema==="por_tramo"?" on":""}`} onClick={()=>setTipoEsquema("por_tramo")}>
            📊 Por Tramo (% sobre ventas totales)
          </button>
          <button className={`toggle-btn${tipoEsquema==="marginal"?" on":""}`} onClick={()=>setTipoEsquema("marginal")}>
            📈 Marginal (% solo sobre el tramo incremental)
          </button>
        </div>
        <div className="infobox" style={{marginBottom:0}}>
          {tipoEsquema==="por_tramo"
            ?'✅ Por Tramo: si vendes $200M y tu tramo es 10M–∞ al 1%, la comisión es $200M × 1% = $2.000.000. El % aplica sobre el TOTAL de las ventas según el tramo en que caíste.'
            :'📈 Marginal: la comisión se calcula por cada "pedazo" de ventas dentro de su tramo. Ej: tramo 0–5M al 3% + tramo 5M–10M al 5% + tramo 10M–∞ al 8%. Cada tramo acumula su parte.'}
        </div>
      </div>
...
function SimuladorTab({ employees, beParams, schemas, globalSchema, tipoEsquema, simEmpId, setSimEmpId, simVentas, setSimVentas }) {
  const empId=simEmpId, setEmpId=setSimEmpId;
  const ventas=simVentas, setVentas=setSimVentas;
  const emp=employees.find(e=>e.id===empId);

  // FIX 4: Commissions are prestacional — calculate full employer burden
  const calc=useMemo(()=>{
    if(!emp) return null;
    const v=+ventas||0;
    ...
    // FIX 4: Las comisiones son factor salarial → generan carga prestacional y parafiscal adicional
    // EXCEPCIÓN: prestación de servicios — no hay carga, se paga solo el honorario variable
    const esPrestacion = emp.categoria === "prestacion";
    const cargaEmpresaComision = (!esPrestacion && emp.comisiones) ? comision*(porcCarga/100) : 0;
    // Empleado regular: aporta 8% sobre base total. Prestación: maneja sus propios aportes
    ...
    const mcPct=pv>0?(pv-cv_)/pv:0;
    // Venta requerida = costo TOTAL de la empresa (incluye comisión + carga sobre comisión) / margen
  ...
  },[emp,ventas,schemas,globalSchema,tipoEsquema,beParams]);
          ...
          <div>
            ...
            <div className="card mb14">
              ...
              {calc.comision>0&&<>
                ...
                {calc.cargaEmpresaComision>0&&<div className="rr" style={{background:"rgba(239,68,68,.04)"}}>
            ...
            {calc.ventaRequerida&&<div className="card">
              <div className="card-title">🎯 Venta Requerida para Cubrir Costo Total de la Empresa</div>
              <div className="infobox" style={{marginBottom:12}}>
                Costo total empresa <strong style={{color:"var(--red)"}}>{fmtCOP(calc.costoEmpresa)}</strong> ÷ margen de contribución <strong style={{color:"var(--green)"}}>{fmtPct(calc.mcPct*100)}</strong> = venta mínima requerida
              </div>
              <div className="stat mb14">
                <div className="stat-l">Venta mínima requerida (cubre salario + cargas + comisión + su carga prestacional)</div>
              ...
              {calc.v>0&&<>
                ...
                <div className="sm mt8" style={{color:calc.v>=calc.ventaRequerida?"var(--green)":"var(--accent)"}}>
                  {calc.v>=calc.ventaRequerida?`✅ Superó por ${fmtCOP(calc.v-calc.ventaRequerida)}`:`⚠️ Falta ${fmtCOP(calc.ventaRequerida-calc.v)} para cubrir el costo total de la empresa`}
                </div>
              </>}
            </div>}
          </div>
    ...
    <div>
      ...
      <div className="g2">
        ...
        <div className="card">
          ...
          <div className="infobox">ℹ️ Necesitas una <strong style={{color:"var(--blue)"}}>API Key</strong> con permisos de escritura desde <a href="https://console.cloud.google.com" target="_blank" style={{color:"var(--blue)"}}>Google Cloud Console</a>. Si prefieres algo más simple, descarga el CSV e impórtalo en Sheets (Archivo → Importar).</div>
...
export default function App() {
  ...
  // FIX 1: All state loads from localStorage (persists on refresh)
  const [employees,setEmployees]=useState(()=>ls("sf_emp",[]));
  const [selEmpId,setSelEmpId]=useState(null);
  const [showAdd,setShowAdd]=useState(false);
  const [editEmpId,setEditEmpId]=useState(null); // FIX 2: Edit support
  ...
  const [tipoEsquema,setTipoEsquema]=useState(()=>ls("sf_tipo","por_tramo")); // FIX 3
  const [beParams,setBeParams]=useState(()=>ls("sf_be",{precioVenta:"",costoVariable:"",otrosCostos:"",margenRentabilidad:""}));
  // Simulator state lifted here so it persists when switching tabs
  ...
  // FIX 1: Save to localStorage on every change
  ...
  const totalCostosFijos=useMemo(()=>employees.reduce((s,e)=>s+calcCostos(e.salario,e.arlRate,e.subsidio,e.categoria).costoTotal,0),[employees]);

  // FIX 2: Open edit modal pre-filled
  const openEdit=emp=>{
  ...
  const saveEmployee=()=>{
    ...
    if(editEmpId){
      // FIX 2: Update existing employee
    ...
    <>
      ...
      <div className="app">
        ...
        <div className="main">
          ...
          {tab==="simulador"&&<SimuladorTab employees={employees} beParams={beParams} schemas={schemas} globalSchema={globalSchema} tipoEsquema={tipoEsquema} simEmpId={simEmpId} setSimEmpId={setSimEmpId} simVentas={simVentas} setSimVentas={setSimVentas}/>}
